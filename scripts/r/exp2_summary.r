...
ggplot(data1 %>% filter(type=="GET"),
  ...
  geom_line(aes(y=response_time_q50), color=color_triad2_dark, size=1) +
  geom_errorbar(aes(ymin=response_time_mean-response_time_confidence_delta,
                    ymax=response_time_mean+response_time_confidence_delta),
                color=color_triad2, width=0.2, size=1) +
  geom_point(color=color_dark) +
...
data_compare <- all_results %>%
  ...
  select(replication_str, servers_str, memaslap, middleware) %>%
  melt(id.vars=c("replication_str", "servers_str")) %>%
  rename(Measurer=variable)
ggplot(data_compare, aes(x=replication_str, y=value, color=Measurer, group=Measurer)) +
  geom_line() +
  geom_point() +
  facet_wrap(~servers_str, ncol=3) +
  ylim(0, NA) +
