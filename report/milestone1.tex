...
When the system starts to saturate, standard deviation also increases. Under normal load (no saturation), all requests enter processing almost immediately. Upon saturation however, each request needs to wait for some time



\todo{Explain high std: Upon examining logs manually, lalala distribution}

\todo{mention that even though stdev goes below 0, in reality it can't be there (physically impossible)}
...
\todo{Explain that MW is functional and can handle a long-running workload without crashing or degrading in performance.}

\todo{Explain the spikes in the graph.}

See Figure~\ref{fig:rep3:throughput}.

\begin{figure}[p]
\centering
\includegraphics[width=\textwidth]{../results/trace_rep3/graphs/throughput.pdf}
\label{fig:rep3:throughput}
\caption{Throughput trace of the middleware as measured by memaslap.}
\end{figure}


\subsection{Response time}

See Figure~\ref{fig:rep3:responsetime}.
