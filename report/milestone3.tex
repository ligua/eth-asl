...
\caption{Throughput predicted using IRTL, as a function of actual throughput calculated from experimental data. The red line shows perfect predictions (the $x=y$ line). Note the horizontal scale does not include 0.}
...
If we assume a nonzero $Z$ and estimate it from the experiments, we get a mean estimated wait time of -0.111ms. Clearly this is impossible: wait time must be non-negative. It could be that memaslap starts the clock for a new request before stopping the clock for the previous request -- which would violate the closed system assumption -- but this is very unlikely.

To explain these results, we need to answer the question: what could be causing the unexpectedly high throughput (caused by a shorter-than-predicted total cycle time)?

\todo{} because total cycle time is higher than it should for given throughput -- but why? since measuring is done by memaslap, probably the problem lies on that side

\todo{} slides: network effects, processing overhead, exceptions and non-normal return values
