...
we have CLOSED queueing network (book: closed queueing network has no external arrivals or departures. As shown in Figure 32.2, the jobs in the system keep circulating from one queue to the next. The total number of jobs in the system is constant.)

we know number of jobs in our case it is concurrency -- or maybe a bit less. 

if analysis would be too complicated, then assume no queue at loadbalancer (good approximation because it takes only a tiny bit of time there)
