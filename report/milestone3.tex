...
We have a closed \todo{closed system AND closed network} queueing network: the total number of jobs in the system is constant. This number is equal to the concurrency parameter $C$ we give to memaslap since each concurrent job in memaslap sends one request and waits for a response before sending the next one.
...
calculate bounds of throughput and response time
