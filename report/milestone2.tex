...
Figure~\ref{fig:exp1:res:responsetime} shows the percentiles of the response time distribution for each parameter set. It is apparent that for all values of $T$, both the median response time (green line) and 95\% quantile (pink line) increase significantly after 216 clients. For this reason, we will exclude all values of $T > 216$ from consideration as unsustainable. Of the remaining setups, the highest throughput is achieved by 216 clients at $T=32$, which is the configuration we will declare optimal at a throughput of 18500 requests per second.
